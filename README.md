...
Utilisez `[lm_loterie id="123"]` pour afficher un résumé d'une loterie spécifique (remplacez `123` par l’ID de l’article). Sans paramètre `id`, le shortcode affiche désormais un catalogue interactif avec filtres dynamiques (statut, catégorie, recherche et tri) qui s'actualise via AJAX. Le paramètre optionnel `sort` permet de définir le tri par défaut (`date_desc`, `date_asc`, `title_asc`, `title_desc`). Pour filtrer par statut, utilisez `status="upcoming"` (ou `active`, `ended`, `cancelled`, `suspended`). Pour n’afficher que les loteries à venir avec une date de début définie, ajoutez `upcoming_date="with_date"` ; pour celles à venir sans date de début, utilisez `upcoming_date="without_date"`. Pour afficher automatiquement la loterie la plus avancée (celle ayant vendu le plus de tickets), utilisez `[lm_loterie id="most_advanced"]`.
